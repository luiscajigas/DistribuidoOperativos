...
async function main() {
    ...
    async function rpcValidateUser(payload) {
        ...
        return new Promise((resolve, reject) => {
            ...
            channel
                .consume("amq.rabbitmq.reply-to", (msg) => {
                if (!msg)
                    return;
                const m = msg;
                ...
                if (consumerTag)
                    channel.cancel(consumerTag).catch(() => undefined);
                try {
                    const parsed = JSON.parse(m.content.toString("utf8"));
                    resolve(parsed);
                }
                catch (e) {
                    reject(e);
    ...
    if (tlsCertPath && tlsKeyPath) {
        const server = https.createServer({ cert: fs.readFileSync(tlsCertPath), key: fs.readFileSync(tlsKeyPath) }, app);
